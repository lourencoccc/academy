...
unsigned long Debug_Index; // debug array index

void Delay(void);
void delayms(unsigned long ms);
void Toggle_Led(void);
void Debug_Dump(unsigned long last);
...
int main(void){  unsigned long last;
  PortF_Init();   // initialize PF1 to output
  SysTick_Init(); // initialize SysTick, runs at 16 MHz
  Debug_Index = 0;          // array index
  last = NVIC_ST_CURRENT_R;
  while(1){
		SW1 = PF4;
		SW2 = PF0;
		
		//The Switchs has a negative logic
		if(!SW1&&!SW2){                     // both pressed
			Toggle_Led();
			delayms(50);			// 100ms delay makes 10Hz period
		} else{                           
			if(!SW1&&(SW2)){                // just SW1 pressed
				Toggle_Led();
				delayms(50);			// 50ms delay makes 20Hz period
			} else{                        
...
void Debug_Dump(unsigned long last){
	...
	if(Debug_Index <50){
		...
		Time[Debug_Index] = (last-now)&0x00FFFFFF;  // 24-bit time difference
		Data[Debug_Index] = GPIO_PORTF_DATA_R&0x02; // record PF1
		last = now;
		Debug_Index++;
	}
}
